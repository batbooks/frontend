...
const PlaylistDetailPage = () => {
  ...
  return (
    <>
      <Navbar />
      <div dir="rtl" className="min-h-screen mt-20 bg-[#a3d5ff] mx-12 p-8 pb-14 rounded-2xl">
        <button
          onClick={handleBack}
              ...
              <table className="min-w-full bg-white">
                <thead>
                  <tr className="bg-gray-100 text-gray-800">
                    ...
                    <th className="py-3 px-4 text-right">جلد کتاب</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {sortedBooks.map((book) => (
                    <tr key={book.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-4 px-4 font-medium text-gray-800">{book.title}</td>
                      <td className="py-4 px-4 text-gray-600">{book.author}</td>
                      <td className="py-4 px-4">
                        <div className="flex items-center">
                          <span className="text-yellow-500 mr-1">{book.rating.toFixed(1)}</span>
