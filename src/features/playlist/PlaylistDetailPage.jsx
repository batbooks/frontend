...
const PlaylistDetailPage = () => {
  ...
  const updateRank = async (bookId, newRank) => {
    ...
    try {
      await fetch(
        `http://127.0.0.1:8000/user/playlists/${playlist.id}/books/${bookId}/`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (swappedBook && swappedBook.book.id !== bookId) {
        await fetch(
          `http://127.0.0.1:8000/user/playlists/${playlist.id}/books/${swappedBook.book.id}/`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDeleteBook = async (bookId, playlistId) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/user/playlists/${playlistId}/books/${bookId}/`,
        {
          method: "DELETE",
          headers: {
            Authorization: auth,
