...
export default function UserDashboard() {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      try {
        const response = await fetch(
          `https://batbooks.liara.run/comments/user/reviews/`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
    ...
    const fetchComments = async () => {
      ...
      try {
        const response = await fetch(
          `https://batbooks.liara.run/comments/user/comments/`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
...
function ReadingBook({
  ...
  const handleDeleteHistoryBook = async () => {
    ...
    try {
      const response = await fetch(
        `https://batbooks.liara.run/book/user-book-progress/${historyId}/`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className="flex flex-col md:flex-row items-center">
          {bookImage === null ? (
            ...
            <img
              ...
              className={`shadow-lg shadow-[#000000]/25 rounded-xl lg:rounded-[20px] w-32 md:w-36 lg:w-[153px] h-40 md:h-44 lg:h-[189px] cursor-pointer`}
              src={`https://batbooks.liara.run${bookImage}`}
              alt="book"
            ></img>
          )}
