...
const GroupChatWindow: React.FC<GroupChatWindowProps> = ({ groupId, groupName,setgroupId }) => {
  /* ------------------------------ Local state ------------------------------ */
  ...
  const { sendJsonMessage, lastJsonMessage, readyState } = useWebSocket(WS_URL, {
    ...
    onClose: () => {console.log("WebSocket disconnected");setgroupId(null)},
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/chat/group/members/${groupId}/`, {
        headers: {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/chat/group/message/${groupId}/`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("access_token")}`,
        },
  ...
  const fetchFollowingUsers = async () => {
    ...
    try {
      ...
      const filtered = data.results.filter((u: any) => !memberIds.includes(u.following_user_id));
    ...
    <div dir="rtl" className="flex flex-col h-full bg-gray-50 rounded-lg shadow-inner">
      ...
      <div className="relative bg-gradient-to-br from-slate-50 via-gray-50 to-stone-100 text-gray-800 p-5 sm:p-6 rounded-xl mx-auto my-8 w-full">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-5 pb-4 border-b border-gray-300 text-center">
          <h2 className="text-md lg:text-xl font-semibold text-sky-700 mb-3 sm:mb-0">گروه {groupName}</h2>
          <div className="flex items-center gap-3">
            ...
            >
              {showMembers ? (
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 ml-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
              ) : (
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 ml-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H7a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
              )}
        ...
        <div className={`transition-all duration-700 ease-in-out overflow-hidden ${showMembers ? "max-h-[40vh] opacity-100 overflow-y-auto" : "max-h-0 opacity-0"}`}>
          {isLoadingMembers ? (
            <div className="flex justify-center py-10"><Loader2 className="animate-spin text-sky-500" size={32}/></div>
          ) : members.length > 0 ? (
            ...
            <div className="text-center py-10 px-4 bg-white/70 rounded-lg border border-dashed border-gray-300 shadow-inner backdrop-blur-sm">
              <h3 className="mt-3 text-md font-semibold text-gray-700">هنوز عضوی در گروه نیست.</h3>
              <p className="mt-1.5 text-sm text-gray-500">با کلیک بر روی دکمه "اضافه کردن"، اعضای جدید را به گروه دعوت کنید.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-4 bg-white border-t border-gray-200">
        <MessageInput newMessage={newMessage} setNewMessage={setNewMessage} onSendMessage={handleSendMessage} isDisabled={!socketConnected}/>
      </div>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-md z-40 flex justify-center items-center p-4">
          <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-6">
              ...
              <button onClick={() => setAddMembersPopUp(false)} className="text-gray-400 hover:text-gray-600 p-1 rounded-full hover:bg-gray-100">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg>
              </button>
            </div>

            <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-2">
              {isLoadingFollowings && <div className="flex justify-center p-4"><Loader2 className="animate-spin text-sky-500" size={24}/></div>}
              {!isLoadingFollowings && filteredUsers.length === 0 && (
                <p className="text-center text-gray-500 p-4">کاربری برای افزودن یافت نشد.</p>
              )}
              {!isLoadingFollowings && filteredUsers.map((u) => (
                <label key={u.id} className="flex items-center gap-3 p-2 hover:bg-gray-100 cursor-pointer">
                  <input type="checkbox" checked={selectedUserIds.includes(u.following_user_id)} onChange={() => toggleSelectUser(u.following_user_id)} className="h-5 w-5 accent-sky-600 rounded border-gray-300"/>
                  {u.following_image ? (
                    <img src={u.following_image} alt={u.following} className="w-8 h-8 rounded-full object-cover" loading="lazy"/>
                  ) : (
                    <UserCheck size={20} className="text-gray-400" />
                  )}
                  <span className={`truncate text-md ${selectedUserIds.includes(u.following_user_id) ? "text-sky-700 font-medium" : "text-gray-700"}`}>{u.following}</span>
                </label>
              ))}
            </div>
            ...
            {filteredUsers.length > 0 && (
              <div className="mt-8 pt-6 border-t border-gray-200 flex flex-col sm:flex-row gap-3">
                <button onClick={handleAddSelectedMembers} disabled={selectedUserIds.length === 0} className={`w-full flex-grow px-6 py-3 rounded-lg font-medium shadow-md transition-all ${selectedUserIds.length === 0 ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-sky-600 text-white hover:bg-sky-700 focus:ring-2 focus:ring-sky-500 transform hover:scale-105"}`}>
                  افزودن {selectedUserIds.length > 0 ? `(${selectedUserIds.length}) کاربر` : "کاربران انتخاب شده"}
                </button>
              </div>
            )}
          </div>
